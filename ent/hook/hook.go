...
// The TagUserFunc type is an adapter to allow the use of ordinary
// function as TagUser mutator.
type TagUserFunc func(context.Context, *ent.TagUserMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TagUserFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TagUserMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TagUserMutation", m)
}
